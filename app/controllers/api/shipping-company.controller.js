...
const { ShippingCompanyService } = require('../../services/shipping-company.service')
...
class ShippingCompanyController {
	constructor() {
		_service = new ShippingCompanyService()
	}
	...
	async create(req, res) {
		try {
			...
			Log4js.error(`[action: ShippingCompanyController create][msg: ${error.message}][file:${__filename}]`)
	...
	async update(req, res) {
		try {
			...
			Log4js.error(`[action: ShippingCompanyController update][msg: ${error.message}][file:${__filename}]`)
	...
	async delete(req, res) {
		try {
			...
			Log4js.error(`[action: ShippingCompanyController delete][msg: ${error.message}][file:${__filename}]`)
	...
	async one(req, res) {
		try {
			...
			Log4js.error(`[action: ShippingCompanyController one][msg: ${error.message}][file:${__filename}]`)
	...
	async all(req, res) {
		try {
			...
			Log4js.error(`[action: ShippingCompanyController all][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
	...
	async list(req, res) {
		try {
			...
			Log4js.error(`[action: ShippingCompanyController list][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
}

module.exports.ShippingCompanyController = ShippingCompanyController
